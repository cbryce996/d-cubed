...
template <> struct point_traits<Point> {
	...
	static coordinate_type get (const Point& p, const orientation_2d& orient) {
...
std::vector<Region> WorldGeometry::generate_regions (
	const std::vector<Vec2>& points, float width, float height
) {
	...
	for (const auto& it : vd.cells ()) {
		...
		regions[region_id].neighbors.assign (
			neighbor_set.begin (), neighbor_set.end ()
		);
		...
		if (!polygon_points.empty ()) {
			...
			std::sort (
				polygon_points.begin (), polygon_points.end (),
				[&] (const Vec2& a, const Vec2& b) {
					float angleA = std::atan2 (
						a.y - centroid.y, a.x - centroid.x
					);
					float angleB = std::atan2 (
						b.y - centroid.y, b.x - centroid.x
					);

...
void RenderGraph::topological_sort() {
    ...
    std::function<void(const std::string&)> visit = [&](const std::string& name) {
        if (on_stack.contains(name)) {
            SDL_LogError(SDL_LOG_CATEGORY_RENDER, "Cycle detected in render graph at pass: %s", name.c_str());
        ...
        if (it == render_passes.end()) {
            SDL_LogError(SDL_LOG_CATEGORY_RENDER, "Unknown render pass dependency: %s", name.c_str());

...
inline void sample_unit_sphere (
	float radius, int lat_steps, int long_steps, std::vector<Vector3>& vertices
) {
	assert (radius > 0.0f);
	assert (lat_steps >= 2);
	assert (long_steps >= 3);
	...
	for (int row = 1; row < lat_steps; ++row) {
		const float phi = static_cast<float> (M_PI) * static_cast<float> (row)
						  / static_cast<float> (lat_steps);

		for (int col = 0; col <= long_steps; ++col) {
			const float theta = 2.0f * static_cast<float> (M_PI)
								* static_cast<float> (col)
								/ static_cast<float> (long_steps);
...
inline void
build_indices (int lat_steps, int long_steps, std::vector<uint32_t>& indices) {
	assert (lat_steps >= 2);
	assert (long_steps >= 3);
